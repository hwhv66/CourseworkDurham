...
void updateBody() {
  ...
  double localMinDx = std::numeric_limits<double>::max();
  double localMaxV = 0.0;
  ...
  int mission_nums = NumberOfBodies / n_tasks;
  int start = this_task * (mission_nums + 1);
  int end = (this_task + 1) * (mission_nums + 1);
  
  if (end > NumberOfBodies) {
    end = NumberOfBodies;
  }
  
  if (start > NumberOfBodies) {
    entry = false;
  }
  ...
  if (entry) {
    for (int i=start; i<end; i++) {
      ...
      for (int j=0; j<NumberOfBodies; j++) {
        if (i!=j) {
          ...
          localMinDx = std::min( localMinDx,distance );
    ...
    MPI_Allreduce(&localMinDx, &minDx, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
    ...
    for (int i=start; i<end; i++) {
      ...
      localMaxV = std::max(localMaxV,thisV);
    }
      
    MPI_Allreduce(&localMaxV, &maxV, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
