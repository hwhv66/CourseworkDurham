...
void updateBody() {
  ...
  double* tempDistance = new double[NumberOfBodies];
  ...
  for (int i = 0; i != NumberOfBodies; i++) {
    ...
    force1[i] = 0.0;
    force2[i] = 0.0;
    tempDistance[i] = std::numeric_limits<double>::max();
    tempV[i] = 0.0;
  }
  ...
  for (int i=0; i<NumberOfBodies - 1; i++) {
    for (int j = i + 1; j<NumberOfBodies; j++) {
        ...
        force0[j] += (x[i][0]-x[j][0]) * quantity / distance ;
        force1[j] += (x[i][1]-x[j][1]) * quantity / distance ;
        force2[j] += (x[i][2]-x[j][2]) * quantity / distance ;
    }
    for (int j = 0; j != NumberOfBodies; j++) {
        minDx = std::min( minDx,tempDistance[j]);
    }
  }
